...
export function useAudioEngine({
  ...
  useEffect(() => {
    ...
    const startMeterCore = async (ctx, sourceNode, meta) => {
      const { stream, pcmSource, statusMain, statusInput } = meta;
      ...
      audioRef.current = { ctx, stream, wklt, pcmSource, mode: stream ? "browser" : "tauri" };
      setStatus(statusMain);
      setStatus2(statusInput);
      tick();
    };
    ...
    const init = async () => {
      try {
        if (isTauri()) {
          ...
          const ctx = new Ctor({
            sampleRate: pick.defaultSampleRate,
            latencyHint: "interactive",
          });
          await ctx.audioWorklet.addModule("/worklets/pcm-source.js");
          const pcmSource = new AudioWorkletNode(ctx, "pcm-source", {
            numberOfInputs: 0,
            numberOfOutputs: 1,
          ...
          await startMeterCore(ctx, pcmSource, {
            stream: null,
            pcmSource,
            statusMain,
            statusInput: `Input: ${pick.label}`,
          });
          if (!mounted) return;
          await startAudioCapture({
            ...
            onPcmBytes: (bytes) => {
              ...
              if (!ref?.pcmSource) return;
              if (bytes.byteLength < 12) return;
              const dv = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
              const frameCount = dv.getUint32(8, true);
              const ch = dv.getUint16(4, true);
              const sampleCount = frameCount * ch;
              if (bytes.byteOffset + 12 + sampleCount * 4 > bytes.byteLength) return;
              const floats = new Float32Array(sampleCount);
              floats.set(new Float32Array(bytes.buffer, bytes.byteOffset + 12, sampleCount));
              try {
                ref.pcmSource.port.postMessage({ type: "pcm", buffer: floats.buffer }, [floats.buffer]);
              } catch (_) {}
            },
          });
          return;
        ...
        await startMeterCore(ctx, src, {
          stream,
          pcmSource: null,
          statusMain: "Monitoring live input",
          statusInput: `Input: ${stream.getAudioTracks()[0]?.label || "System default microphone"}`,
        });
      } catch (err) {
        setRunning(false);
        setSelectedOffset(-1);
