...
克隆或下载本仓库后，在项目根目录启动任意**静态文件服务**，再打开 `audiometer.html`。
...
浏览器访问：`http://localhost:8080/audiometer.html`（端口以实际为准）。
...
- **第三方资源**：页面通过 Google Fonts 加载字体；若部署环境对出站请求敏感，可改为自托管字体并修改 `audiometer.html` 中的 `<link>`。
